...
function wc_brainblocks_add_to_gateways( $gateways ) {
	$gateways[] = 'WC_Gateway_BlockVerse';
	return $gateways;
}
add_filter( 'woocommerce_payment_gateways', 'wc_blockverse_add_to_gateways' );
/**
 ...
 */
function wc_blockverse_gateway_plugin_links( $links ) {
	$plugin_links = array(
		'<a href="' . admin_url( 'admin.php?page=wc-settings&tab=checkout&section=blockverse_gateway' ) . '">' . __( 'Configure', 'wc-gateway-blockverse' ) . '</a>'
	);
	return array_merge( $plugin_links, $links );
}
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'wc_blockverse_gateway_plugin_links' );
/**
 ...
 * @author 		SkyVerge
 */
add_action( 'plugins_loaded', 'wc_blockverse_gateway_init', 11 );
function wc_blockverse_gateway_init() {
	class WC_Gateway_BlockVerse extends WC_Payment_Gateway {
		/**
		 * Constructor for the gateway.
		 */
		public function __construct() {
	  
			$this->id                 = 'blockverse_gateway';
			$this->icon               = apply_filters('woocommerce_offline_icon', '');
			$this->has_fields         = false;
			$this->method_title       = __( 'BlockVerse', 'wc-gateway-blockverse' );
            $this->method_description = __( 'Allows Crypto Payments on BlockVerse.', 'wc-gateway-blockverse' );
            $this->order_button_text  = __( 'blockverse', 'wc-gateway-blockverse' );
            ...
            add_filter('woocommerce_order_button_html',array( $this, 'display_blockverse_button_html' ),1);
		...
		public function init_form_fields() {
			...
			$this->form_fields = apply_filters( 'wc_brainblocks_form_fields', array(
				...
				'enabled' => array(
					'title'   => __( 'Enable/Disable', 'wc-gateway-blockverse' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable RaiBlocks Payments via brainblocks.io', 'wc-gateway-blockverse' ),
					'default' => 'yes'
				),
				
				'title' => array(
					'title'       => __( 'Title', 'wc-gateway-blockverse' ),
					'type'        => 'text',
					'description' => __( 'This controls the title for the payment method the customer sees during checkout.', 'wc-gateway-blockverse' ),
					'default'     => __( 'RaiBlocks Payment', 'wc-gateway-blockverse' ),
					'desc_tip'    => true,
				),
				
				'description' => array(
					'title'       => __( 'Description', 'wc-gateway-blockverse' ),
					'type'        => 'textarea',
					'description' => __( 'Payment method description that the customer will see on your checkout.', 'wc-gateway-blockverse' ),
					'default'     => __( 'Pay with RaiBlocks via brainblocks.io.', 'wc-gateway-blockverse' ),
					'desc_tip'    => true,
				),
				
				'instructions' => array(
					'title'       => __( 'Instructions', 'wc-gateway-blockverse' ),
					'type'        => 'textarea',
					'description' => __( 'Pay with RaiBlocks via brainblocks.io.', 'wc-gateway-blockverse' ),
					'default'     => '',
					'desc_tip'    => true,
                ),
                
                'destination' => array(
					'title'       => __( 'RaiBlocks Address', 'wc-gateway-blockverse' ),
					'type'        => 'text',
					'description' => __( 'Address to receive any sent RaiBlocks', 'wc-gateway-blockverse' ),
					'default'     => __( '', 'wc-gateway-blockverse' ),
					'desc_tip'    => true,
				),
			) );
        }
